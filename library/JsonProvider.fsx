(**
// can't yet format YamlFrontmatter (["category: Type Providers"; "categoryindex: 1"; "index: 3"], Some { StartLine = 2 StartColumn = 0 EndLine = 5 EndColumn = 8 }) to pynb markdown
...
let docAsync =
  WorldBank.AsyncLoad(wbReq)(* output: 
val wbReq : string =
...
for record in doc.Array do
  record.Value |> Option.iter (fun value ->
    printfn "%d: %f" record.Date value)(* output: 
...
printfn "%s (retweeted %d times)\n:%s"
  tweet.User.Value.Name tweet.RetweetCount.Value tweet.Text.Value(* output: 
Marin (retweeted 0 times)
:大金払って遊ばしてもらう。もうこの上ない贅沢。でも，実際にはその贅沢の本質を満喫できる人は限られてる。そこに目に見えない階層があるとおもう。
type Tweet = JsonProvider<...>
val text : string =
  " {"in_reply_to_status_id_str":null,"text":"\u5927\u91d1\u6255"+[2255 chars]
val tweet : JsonProvider<...>.Root =
  {
  "in_reply_to_status_id_str": null,
  "text": "大金払って遊ばしてもらう。もうこの上ない贅沢。でも，実際にはその贅沢の本質を満喫できる人は限られてる。そこに目に見えない階層があるとおもう。",
...
// can't yet format Span ([DirectLink ([Literal ("JSON Parser", Some { StartLine = 10 StartColumn = 3 EndLine = 11 EndColumn = 14 })], "JsonValue.html", None, Some { StartLine = 10 StartColumn = 3 EndLine = 11 EndColumn = 40 }); Literal (" - provides more information about
working with JSON values dynamically.", Some { StartLine = 10 StartColumn = 3 EndLine = 11 EndColumn = 76 })], Some { StartLine = 10 StartColumn = 0 EndLine = 10 EndColumn = 66 }) to pynb markdown
// can't yet format Span ([Literal ("API Reference: ", Some { StartLine = 12 StartColumn = 3 EndLine = 12 EndColumn = 18 }); DirectLink ([Literal ("JsonProvider", Some { StartLine = 12 StartColumn = 19 EndLine = 12 EndColumn = 50 })], "https://fsprojects.github.io/FSharp.Data/reference/fsharp-data-jsonprovider.html", None, Some { StartLine = 12 StartColumn = 19 EndLine = 12 EndColumn = 50 })], Some { StartLine = 10 StartColumn = 0 EndLine = 10 EndColumn = 66 }) to pynb markdown
// can't yet format Span ([Literal ("API Reference: ", Some { StartLine = 13 StartColumn = 3 EndLine = 14 EndColumn = 18 }); DirectLink ([Literal ("JsonValue", Some { StartLine = 13 StartColumn = 19 EndLine = 14 EndColumn = -3 })], "https://fsprojects.github.io/FSharp.Data/reference/fsharp-data-jsonvalue.html", None, Some { StartLine = 13 StartColumn = 19 EndLine = 14 EndColumn = -3 })], Some { StartLine = 10 StartColumn = 0 EndLine = 10 EndColumn = 66 }) to pynb markdown
*)
