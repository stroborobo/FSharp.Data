(**
// can't yet format YamlFrontmatter (["category: Type Providers"; "categoryindex: 1"; "index: 2"], Some { StartLine = 2 StartColumn = 0 EndLine = 5 EndColumn = 8 }) to pynb markdown
...
for row in f1Calendar.Rows do
  printfn "Race, round %A is hosted at %A on %A"
    row.Round row.``Grand Prix`` row.Date(* output: 
*)
(**
...
val rawStats : HtmlProvider<...>.Table3
val getMinorVersion : v:string -> string
val stats : (string * decimal) [] =
  [|("4.2", 51224M); ("4.1", 67252M); ("4.0", 38023M); ("3.3", 666829M);
    ("3.2", 16392M); ("3.1", 183656M); ("3.0", 351785M); ("2.4", 397232M);
    ("2.3", 467896M); ("2.2", 281129M); ("2.1", 26920M); ("2.0", 81019M);
    ("1.1", 72232M); ("1.0", 20520M)|]*)
(**
...
// can't yet format Span ([DirectLink ([Literal ("HTML Parser", Some { StartLine = 4 StartColumn = 3 EndLine = 6 EndColumn = 14 })], "HtmlParser.html", None, Some { StartLine = 4 StartColumn = 3 EndLine = 6 EndColumn = -2 }); Literal (" - provides more information about
working with HTML documents dynamically.", Some { StartLine = 4 StartColumn = 3 EndLine = 6 EndColumn = 79 })], Some { StartLine = 4 StartColumn = 0 EndLine = 4 EndColumn = 67 }) to pynb markdown
*)
