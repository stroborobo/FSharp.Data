(**
// can't yet format YamlFrontmatter (["category: Type Providers"; "categoryindex: 1"; "index: 1"], Some { StartLine = 2 StartColumn = 0 EndLine = 5 EndColumn = 8 }) to pynb markdown
...
for row in msft.Rows |> Seq.truncate 10 do
  printfn "HLOC: (%A, %A, %A, %A)" row.High row.Low row.Open row.Close(* output: 
...
*)
let small = CsvProvider<"../data/SmallTest.csv", ResolutionFolder=ResolutionFolder>.GetSample()(* output: 
val small : CsvProvider<...>*)
(**
We can also use the default constructor instead of the `GetSample` static method:

*)
let small2 = new CsvProvider<"../data/SmallTest.csv", ResolutionFolder=ResolutionFolder>()(* output: 
...
for row in small.Rows do
  ...
  if speed > 15.0M<metre/second> then
    printfn "%s (%A m/s)" row.Name speed(* output: 
...
for row in airQuality.Rows |> Seq.truncate 10 do
  if row.Month > 6 then
    printfn "Temp: %i Ozone: %f " row.Temp row.Ozone(* output: 
...
for r in mortalityNy.Rows do
  if r.``Cause of death Code`` = "V13.4" then
    printfn "%s (%d cases)" r.County r.Count(* output: 
...
*)
CsvProvider<"X,Y,Z\nthis,that,1.0",
            MissingValues="this,that">.GetSample().Rows(* output: 
...
for row in csv.Rows do
  printfn "%f %d %f"
    ...
    row.Foo
    (defaultArg row.Column3 1.0)(* output: 
1.000000 2 3.000000
...
for row in titanic1.Rows |> Seq.truncate 10 do
  printfn "%s Class = %d Fare = %g"
    row.Name row.``Passenger Class`` row.Fare(* output: 
  ...
  airQuality.Map(fun row ->
    AirQuality.Row
      ( row.Ozone * 2.0, row.``Solar.R``,
        row.Wind, row.Temp, row.Month, row.Day))(* output: 
...
let airQualityWithExtraRows =
  airQuality.Append newRows(* output: 
...
myCsv.SaveToString()(* output: 
type MyCsvType = CsvProvider<...>
val myRows : CsvProvider<...>.Row list =
  [(1, "a", None); (2, "B", Some 11/17/2021 3:31:17 PM)]
val myCsv : MyCsvType
val it : string = "1,a,
2,B,2021-11-17T15:31:17.8388598+00:00
"*)
(**
